...
for i, row in enumerate(data, 1):
    gyro_avg[i][0] = x(data[i]["gyro"][0], data[i]["gyro"][0], data[i]["gyro"][0])
    gyro_avg[i][1] = x(data[i]["gyro"][1], data[i]["gyro"][1], data[i]["gyro"][1])
    gyro_avg[i][2] = x(data[i]["gyro"][2], data[i]["gyro"][2], data[i]["gyro"][2])

    accl_avg[i][0] = x(data[i]["accel"][0], data[i]["accel"][0], data[i]["accel"][0])
    accl_avg[i][1] = x(data[i]["accel"][1], data[i]["accel"][1], data[i]["accel"][1])
    accl_avg[i][2] = x(data[i]["accel"][2], data[i]["accel"][2], data[i]["accel"][2])

    magn_avg[i][0] = x(data[i]["magne"][0], data[i]["magne"][0], data[i]["magne"][0])
    magn_avg[i][1] = x(data[i]["magne"][1], data[i]["magne"][1], data[i]["magne"][1])
    magn_avg[i][2] = x(data[i]["magne"][2], data[i]["magne"][2], data[i]["magne"][2])
    ...
    e = accl_avg[i] - a3[i]
    e_magnitude = magnitude(e) #math.sqrt(e[0] ^ 2 + e[1] ^ 2 + e[2] ^ 2) TODO: check ig only 3 parameter work with lamda above
    ...
    qg_m = quat.myQuatNormalize(quat.myQuatProd(qg_m[i], deltaQm))
    qg_a = quat.myQuatNormalize(quat.myQuatProd(qg_a[i], deltaQa))
...
# import json
#
# f = open("Untitled-1.json")
#
#
# data = json.load(f)
#
# # print((data["data"]["c:44756"]["s"][0][0]))
# file_id = 9
# fw = open(str(file_id)+".txt", "a")
# for i in (data["data"]["c:44756"]["s"][file_id]):
#     i = str(i)
#     tmp_i = i.replace("'", "")
#     tmp_i = tmp_i.replace("[", "")
#     tmp_i = tmp_i.replace("]", "")
